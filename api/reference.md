...
In the API, Airtable is an API-key connector with one field, `apiKey`, which takes a [personal access token](https://docs.whalesync.com/connectors/airtable/personal-access-tokens). Create a side with `{"connector": "airtable", "auth": {"apiKey": "pat…"}, "base": "app…"}`, or omit `auth` and `base` to leave it for a person. In the app, signing in to Airtable is the default and the token is the second option, so a person finishing the side in the browser may sign in instead of entering a token. A PATCH of that side's `auth` rotates the token. A side a person connected by signing in can't be switched to a token through the API; create a new side instead. The API only ever sees the token.
